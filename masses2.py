...
with st.expander("📚 How it works: Complete Formula & Variables"):
    ...
    **ΣK = (Z × MASS_P) + (N × MASS_N) - [ (N_alphas × E_ALPHA) + (N_macro × E_MACRO_LINK) + (N_halo × E_LINK) + E_PAIR - JITTER_TAX ]**
    ...
    #### The Halo Rule (Precision Upgrade: 99.92% Accuracy)
    To account for neutron-rich boundaries (the 'halo' of the nucleus), the algorithm calculates sub-alpha primitive structures.
    Integrating this geometric Halo Rule (along with the pure theoretical E_ALPHA baseline) pushed the theoretical mass prediction accuracy to an unprecedented **99.92%** across the entire isotopic matrix (AME2020).
    ...
    * **E_ALPHA (28.320 MeV):** The baseline theoretical compilation discount for one complete Alpha-cluster, derived purely geometrically (12 × 2.360 MeV).
    ...
    * **JITTER_TAX:** The total dynamic noise penalty. Postulated as an irreducible hardware constant representing minimum computational latency (**0.0131 MeV** per open port).
...
with tab1:
    ...
    if not df_masses.empty:
        with st.spinner('Compiling matrix...'):
            ...
            # --- FUTURE ELEMENTS PREDICTIONS ---
            st.markdown("---")
            st.write("### Deterministic Predictions: Trans-Oganesson Elements (Z > 118)")
            st.markdown("""
            Standard models predict an 'Island of Stability' for superheavy elements near Z=120, N=184. 
            Grid Physics applies the exact geometric logic to predict their precise masses and stability vectors. 
            These numbers represent the hypothetical hardware compilation *if* the lattice integrity holds, though theoretical models suggest instantaneous Geometry Overflow.
            """)
            
            future_predictions = [
                (119, 184, "Ununennium-303"), 
                (120, 184, "Unbinilium-304")
            ]
            
            pred_data = []
            for z_p, n_p, name_p in future_predictions:
                m_p = engine.compile_mass(z_p, n_p)
                raw_m_p = (z_p * MASS_P) + (n_p * MASS_N)
                be_p = raw_m_p - m_p
                pred_data.append({
                    "Element (Isotope)": f"{name_p} (Z={z_p}, N={n_p})",
                    "Simureality Mass (MeV)": f"{m_p:.3f}",
                    "Binding Energy (BE)": f"{be_p:.3f}",
                    "BE per Nucleon (MeV/A)": f"{(be_p / (z_p+n_p)):.4f}"
                })
            
            st.dataframe(pd.DataFrame(pred_data), use_container_width=True)
            st.warning("⚠️ **Warning: Geometry Overflow Threshold.** Standard binding energy (BE) per nucleon for stable heavy elements is ~7.5-8.0 MeV/A. These predictions yield ~6.9 MeV/A. This confirms the severe topological debt and non-viability of the theoretical 'Island of Stability' within the discrete FCC vacuum.")
