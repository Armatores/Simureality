...
tab1, tab2 = st.tabs(["Single Core & Matrix", "Hardware Proofs (Z, N ≤ 20)"])
...
with tab1:
    ...
    st.markdown("---")
    st.write("### Global Matrix Log")
    if not df_masses.empty:
        global_df = generate_global_matrix(engine, df_masses)
        st.dataframe(global_df, use_container_width=True, height=400)
...
with tab2:
    ...
    st.markdown("Comparing hardware cache calculations with experimental AME2020 mass spectrometry.")
