...
# --- ПОДГОНОЧНЫЕ КОЭФФИЦИЕНТЫ ВЕЙЦЗЕККЕРА (КЛАССИЧЕСКИЙ ХАРДКОД) ---
A_V = 15.75
A_S = 17.8
A_C = 0.711
A_A = 23.7
...
st.set_page_config(page_title="Grid Physics vs Liquid Drop", layout="wide")
...
class SimurealityBaselineCore:
    """Наш голый 3D-движок без единого подгоночного коэффициента"""
    def __init__(self):
    ...
    def compile_mass(self, Z, N):
        ...
        if n_alphas == 0:
            ...
            if halo_total % 2 != 0: jitter += JITTER_COST
...
# --- RENDER UI ---
st.title("Clash of Paradigms: Grid Physics vs. Liquid Drop Model")
st.markdown("""
**Левая сторона:** Голая геометрия ГЦК-решетки (0 эмпирических подгонок). Только базовая алгебра сборки узлов.
**Правая сторона:** Классическая Капельная Модель Вейцзеккера (5 эмпирических коэффициентов, искусственно подогнанных под AME).
""")
...
st.markdown("---")
st.write("### Глобальная сравнительная матрица (Весь AME2020)")
