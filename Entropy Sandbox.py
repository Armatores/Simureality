...
# =====================================================================
# SIMUREALITY V33.0: PHASE TRANSITION ENGINE (THE HONEST ORACLE)
# 100% Geometry & Tensor Topology. Zero Hardcoded Molecules.
# =====================================================================

st.set_page_config(page_title="V33.0 Phase Engine", layout="wide", page_icon="🧊")
...
def calculate_phase_transitions(f):
    ...
    # Зависит от площади поверхности (Voxel Footprint) и силы P2P сети
    # -------------------------------------------------------------
    base_vacuum_lock = (f["asa"] * 1.6) + (f["mw"] * 0.7)
    
    # Торрент-кэш: Идеально спаренные клиенты(доноры) и серверы(акцепторы) дают мощную сцепку
    ...
    spaghetti_drag = f["rot"] * 9.0 # Перепутанные кабели тяжело вырвать из RAM
    
    t_boil = base_vacuum_lock + p2p_cache + arom_stacking_bonus + spaghetti_drag
    
    if f["heavy"] <= 2 and p2p_cache == 0: t_boil *= 0.35 # Легкие газы без P2P

    # -------------------------------------------------------------
    # 2. ТОЧКА ПЛАВЛЕНИЯ (ZIP-Кристалл -> Жидкость)
    # Экстремально зависит от СИММЕТРИИ. Шары и диски пакуются идеально.
    # -------------------------------------------------------------
    # Если молекула - идеальный шар (CCl4), бонус огромен! КПД упаковки взлетает.
    packing_eff = 0.35 + (f["sphericity"] * 0.45) + (f["disk_ness"] * 0.35)
    if f["rod_ness"] > 0.85 and f["rot"] == 0: packing_eff += 0.3 # Жесткие стержни (CO2) тоже пакуются ровно
    
    # Идеальная P2P симметрия (как у H2O) дает супер-бонус кристаллизации (Лед)
    sym_p2p_bonus = 60.0 if (f["donors"] == f["acceptors"] and f["donors"] > 0) else 0.0
    
    # Вращающиеся хвосты ("спагетти") РУШАТ кристалл (поэтому Гептан замерзает при сверхнизких Т)
    ...
    # Аппаратные замки (Сублимация). Жесткие линии без портов испаряются минуя RAM.
    is_sublime = False
    if f["rod_ness"] > 0.9 and f["asa"] < 60 and p2p_cache == 0 and f["heavy"] > 1:
        t_melt = t_boil * 1.25 # Плавление ВЫШЕ кипения!
        is_sublime = True
    ...
    # -------------------------------------------------------------
    mass_drift = f["donors"] * 50.0 # Clock Drift (Рассинхронизация масс легких H)
    t_deg = 1500.0 + (f["heavy"] * 150.0) + (f["arom"] * 1000.0) - mass_drift - (f["rot"] * 30.0)
    
    if "O=C=O" in f["smiles"] or "N#N" in f["smiles"] or "[HH]" in f["smiles"]:
        t_deg = 6000.0 # Идеальные прямые оси живут вечно
        
    return {"T_melt": round(t_melt), "T_boil": round(t_boil), "T_deg": round(t_deg), "Sublimes": is_sublime}

# --- UI ---
st.title("🧊 V33.0: Absolute Hardware Phase Engine")
st.markdown("Мы удалили все костыли. Матрица вычисляет **честный 3D Тензор Инерции (Sphericity/Disk-ness)** и **Voxel Footprint (ASA)**. Агрегатные состояния — это исключительно алгоритмы кэширования геометрии.")
    ...
    if st.button("🚀 Декомпилировать Агрегатные Состояния", type="primary", use_container_width=True):
        with st.spinner("Рендеринг 3D-геометрии и вычисление Тензора Инерции..."):
            ...
            if features:
                ...
                st.markdown("### 🧬 Извлеченные Аппаратные Метрики (БЕЗ КОСТЫЛЕЙ):")
                c_m1, c_m2, c_m3, c_m4 = st.columns(4)
                c_m1.metric("UDP Footprint (ASA)", f"{features['asa']:.1f} Å²", help="Площадь поверхности. Основа Точки Кипения.")
                c_m2.metric("3D Сферичность", f"{features['sphericity']*100:.1f}%", help="100% = Идеальный шар. Дает гигантский бонус к Плавленью (Упаковка).")
                c_m3.metric("Spaghetti-Фактор", features['rot'], help="Вращающиеся связи. Намертво рушат кристалл, понижая Плавление.")
                c_m4.metric("P2P Сеть (D/A)", f"{features['donors']} / {features['acceptors']}", help="Доноры и Акцепторы. Идеальная симметрия (2/2) цементирует Кристалл.")
                
                st.markdown("---")
                c1, c2, c3 = st.columns(3)
                tm_label = f"{phases['T_melt']} K (Сублимация)" if phases['Sublimes'] else f"{phases['T_melt']} K"
                c1.metric("🧊 Плавление (ROM Cache)", tm_label, help="Фиксация статических указателей P2P и геометрическая упаковка.")
                c2.metric("💧 Кипение (RAM Cache)", f"{phases['T_boil']} K", help="Динамическая перемаршрутизация пакетов.")
                c3.metric("🔥 Пиролиз (Kernel Panic)", f"{phases['T_deg']} K", help="Предел прочности ковалентной оси.")
