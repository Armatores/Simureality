...
# --- СТРОГИЕ АППАРАТНЫЕ КОНСТАНТЫ SIMUREALITY (UNIVERSAL HALO) ---
MASS_P = 938.272
MASS_N = 939.565
E_ELECTRON = 0.511
E_ALPHA = 28.32       
...
st.set_page_config(page_title="Universal Core Compiler", layout="wide")

@st.cache_data
def load_ame_masses(filename="mass.txt"):
    """
    УНИВЕРСАЛЬНЫЙ ПАРСЕР. 
    Фильтры '#' и '*' отключены. Парсим весь массив AME (вкл. экзотику и экстраполяции).
    """
    data = []
    try:
        with open(filename, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Жестко вычищаем символы синтетики для конвертации в float
...
class SimurealityBaselineCore:
    """Универсальный движок сборки масс (вкл. экзотику и мультиплексирование гало)"""
    def __init__(self):
    ...
    def compile_mass(self, Z, N):
        ...
        # --- УНИВЕРСАЛЬНАЯ ЛОГИКА ГАЛО (ПРОТОН-НЕЙТРОН И ЭКЗОТИКА) ---
        if rem_Z > 0 and rem_N > 0:
            # 1. Хабы: Миноритарные нуклоны выступают в качестве хабов
            hubs = min(rem_Z, rem_N)
            # 2. Клиенты: Мажоритарные нуклоны делят (мультиплексируют) хабы
            clients = max(rem_Z, rem_N)
            
            # Хабы дают базовый профит E_LINK, плюс спиновый бонус E_PAIR за p-n пару
            binding_halo += hubs * (E_LINK + E_PAIR)
            
            # Клиенты занимают по 1 порту хаба. У каждого клиента остается 11 открытых портов.
            open_ports = clients * 11
            jitter += open_ports * JITTER_COST
            
        elif rem_Z > 0 or rem_N > 0:
            # Чистая экзотика: Хабов нет (только протоны ИЛИ только нейтроны)
            orphans = rem_Z + rem_N
            # Сироты светят всеми 12 портами в вакуум
            open_ports = orphans * 12
            jitter += open_ports * JITTER_COST

        total_binding = binding_alphas + binding_macro + binding_halo - jitter
        return (Z * MASS_P) + (N * MASS_N) - total_binding
...
# --- RENDER UI ---
st.title("Clash of Paradigms: Grid Physics vs. Liquid Drop (Unfiltered AME2020)")
st.markdown("""
**Обновление V2.0:** - Фильтры синтетических данных AME отключены. Скрипт анализирует ВЕСЬ массив изотопов.
- Внедрена универсальная логика мультиплексирования портов Гало (Без эмпирических костылей). 
""")

df_masses = load_ame_masses("mass.txt")
grid_engine = SimurealityBaselineCore()
...
target_N = st.sidebar.number_input("Нейтроны (N)", min_value=0, max_value=184, value=3, step=1)
...
if not df_masses.empty and (target_Z, target_N) in df_masses.index:
    ...
    col2.metric(label="Grid Physics ΣK", value=f"{grid_mass:.3f} MeV", delta=f"{grid_err:.3f} MeV", delta_color="inverse")
...
if not df_masses.empty:
    with st.spinner('Анализ таблицы Менделеева...'):
