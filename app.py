...
# ==========================================
# SIMUREALITY APP V2.0 (FULL DATASET)
# ==========================================

st.set_page_config(page_title="Simureality Nuclear Calculator", layout="wide", page_icon="⚛️")

# --- ПОЛНАЯ БАЗА ДАННЫХ (ИЗ ТВОЕГО ФАЙЛА EN 249 MEGA TEST) ---
DATASET = [
...
# Создаем DataFrame для справочника
df_ref = pd.DataFrame(DATASET, columns=["Изотоп", "Z (Протоны)", "A (Масса)", "Real BE"])
df_ref["N (Нейтроны)"] = df_ref["A (Масса)"] - df_ref["Z (Протоны)"]
...
with st.sidebar:
    ...
    st.markdown("Полная база: **215+ изотопов**")
    
    # Виджет поиска
    search_term = st.text_input("Поиск по имени (например, Fe)", "")
    
    # Фильтрация таблицы
    if search_term:
        filtered_df = df_ref[df_ref['Изотоп'].str.contains(search_term, case=False)]
    else:
        filtered_df = df_ref
        
    st.dataframe(
        filtered_df[["Изотоп", "Z (Протоны)", "N (Нейтроны)", "Real BE"]], 
        height=600,
        hide_index=True
    )
...
*Алгоритм Simureality рассчитывает энергию связи через геометрию решетки (FCC Lattice), 
предсказывая свойства {len(DATASET)} изотопов без подгонки под ответ.*
""")
    ...
    if st.button("🚀 Рассчитать Энергию", type="primary", use_container_width=True):
        ...
        with col_res2:
            if ref_data:
                ...
                st.info(f"ℹ️ Изотоп (Z={Z_input}, N={N_input}) не найден в справочнике, но расчет выполнен.")
        ...
        if ref_data:
            ...
            if accuracy > 99.0:
                st.success("🎯 **Высокая точность!** Геометрическая модель совпадает с реальностью.")
        else:
            st.metric("Simureality Prediction", f"{sim_be:.2f} MeV")
            st.info("Расчет выполнен ab initio (с нуля).")
